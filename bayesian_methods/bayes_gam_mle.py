...
def model(X,Y):
    N,D_X=X.shape
    alpha = numpyro.sample("alpha", dist.Normal(alpha_mean, 0.1*np.ones((1, 1))))#0.001
    beta = numpyro.sample("beta", dist.Normal(beta_mean,0.1*jnp.ones((D_X, 1))))#0.001
...
if __name__ == "__main__":
    ...
    np.save("./npy_files/bayes_gam_mle.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"divergences":divergences})
